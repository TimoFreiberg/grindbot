...
**Handoff:** `grindbot handoff done --commit <hash>` validates the commit exists and is ahead of base, writes `result.json`, and resets the stop counter. `grindbot handoff needs-feedback --message <text>` (or `--message-file <path>`) writes the result directly. The handoff binary walks up to the nearest `.jj` directory — the workspace root, not the main repo.
